	...
	*/
	...
	findFiltered(trees, node, key) {
		if (!node.nodes) {
			...
			const filteredSubFolder = node.nodes.length ? node.nodes.reduce((p, c, k) => {
	...
	setDisplayTree(tree, prevs, curr, keyPath) {
		...
		if (!curr.nodes) {
			...
			const filteredChildren = (
				parentNode.nodes.some(child => child.isSearchDisplay === true)
				?
				parentNode.nodes.filter(child => child.isSearchDisplay === true)
				:
				parentNode.nodes
			);
			const itemKey = "treeModel-leaf-" + curr.id;
		...
		/*the node*/
			...
			const key = "treeModel-node-" + currLevel + "-" + curr.id;
			...
			if ((!curr.isOpen && !isSearching) || (!curr.isSearchOpen && isSearching)) {
				...
				if (shouldDisplay) {
					...
					const nodesList = curr.nodes.length ? curr.nodes.reduce((p, c, k) => {
						if (c === undefined || k === undefined) {
							return p;
						}
						return this.setDisplayTree(tree, p, c, keyPath + ".nodes." + k);
					}, []) : [];


					if (nodesList.length > 0) {
						openedNode.push(
							<ul key={"treeModel-nodes-list" + currLevel}>
								{nodesList}
							</ul>
						);
					}
	...
	render() {
		...
		return (
			<div className="treeModel-container">
				{headerContent}
				<div className="treeModel-display-tree-container">
					{bodyContent}
				</div>
			</div>
		);
